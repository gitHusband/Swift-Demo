    ...
    private func createImage() {
        ...
        for i in 0..<self.imageList!.count {
            ...
            if self.placeholder != nil {
                img = placeholder!
                self.webImgDic[urlStr] = img.pngData()
            }
            if urlStr.hasPrefix("http") {// 图片来自网络
                ...
                if localImg != nil {
                    self.webImgDic[urlStr] = localImg!.pngData()
                }
            }
        }
    }
    ...
    private func createLoopView() {
        ...
        for i in 0..<3 {
            ...
            if self.placeholder != nil {
                ...
                }else if webImgDic[self.imageList![i] as! String]! == self.placeholder!.pngData() {
    ...
    private func loadWebImage(url: String?) {
        ...
        if self.webImgDic[url!] != nil {
            if self.placeholder != nil {// 如果placeholder不为nil，需要判断存在Dic中的是不是占位图
                guard self.webImgDic[url!] == self.placeholder!.pngData() else {
                    return
                }
            }
        }
